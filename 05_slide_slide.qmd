...
- 著者が1名、かつ氏名のみ表記なら`author: 氏名`だけでもOK
...
- 個別のチャンクオプション[（`#| オブション: ...`）]{.small}は`knitr: opts_chunk`より優先する
...
Quartoのマークダウン記法では左、中央、右のみ[（`{fig-align=}`）]{.small}

- `{.absolute}`で任意の位置に図を表示可能[（`top`、`left`等で位置調節; 単位はpx）]{.small}
...
`format: beamer`を使用^[実習環境がmybinder.orgの場合、コンパイルできない可能性がある]

- [reveal.jsとは見た目が大きく変わる]{.alert}ことに注意

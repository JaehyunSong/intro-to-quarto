...
- ファイルのサイズが非常に大きくなるので注意
   ...
   - `self-contained-math`：数式レンダリングエンジン[（MathJax等）]{.small}を埋め込む
      - ファイルサイズがかなり大きくなるため注意^[ネットが使えない環境で閲覧する必要がある文書なら`true`が必須だが、それ以外のケースだと`false`（既定値）を推奨]
...
再現・再生用のデータセットとコード[（またはQuartoファイル）]{.small}、+ $\alpha$を格納したGitHubレポジトリをベースに分析環境を生成するサービス

- GitHubレポジトリの中身
   - `apt.txt`、`environment.yml`、`postBuild`、分析用のコード、データなど
      - `apt.txt`内に必要なライブラリを記述
      - `environment.yml`内にRおよびパッケージの情報を記述
   - ただし、すべてのパッケージ情報を記述することは非効率的
- サンプルレポジトリ：<https://github.com/JaehyunSong/quarto_binder>
- 詳細は[公式ホームページ](https://jupyter.org/binder)を参照

## {renv} $\times$ mybinder.org

詳細は<https://quarto.org/docs/projects/binder.html>を参照

1. RStudioで新規プロジェクト（[Quarto Project]{.alert}）を作成 <!-- 使い勝手は変わらない -->
   - Use renv with this projectも選択
2. 公開の前に{renv}で（`renv::snapshot()`）環境を保存
3. Terminalペインで`quarto user binder`
4. 任意のGithubレポジトリを作成し、プロジェクトフォルダーの中身をそのままアップロード <!-- 不要なやつもそこそこあるが... -->
   - `rnev`フォルダー内の`library`フォルダーはアップロードしなくても良い
5. `https://mybinder.org/v2/gh/GitHubアカウント名/レポジトリ名/HEAD?urlpath=rstudio`
   - 最初の立ち上げはかなり時間がかかるため、他人に共有する前に自分で立ち上げておくこと

<!-- renv.lockには現在使われているパッケージのみ記録される -->

## {renv} $\times$ mybinder.org：注意点

1. プロジェクトごとgitで管理することを推奨[（gitの解説は省略）]{.small}
2. GitHubアップロード[（または、プッシュ）]{.small}時、`.Rprofile`も忘れないこと
   - `.Rprofile`は隠しファイル
3. `apt.txt`は自分で書く必要がある
   - `apt.txt`に書かれたライブラリはサーバー立ち上げ[（Dockerイメージ生成）]{.small}時、一緒にインストールされる
   - mybinder.orgではサーバーが立ち上がると`sudo`コマンドが使えない
   - 一旦、動作を確認し、必要なライブラリがあればそれを`apt.txt`に書いておく

#### 結論

- 2026年2月現在、2つを組み合わせて使うメリットは少ない
   - {renv}は分析当時の環境を保存する自分用のパッケージ
   - ただし、Binderの仕組みを理解しているのであれば、使える
   - すくなくとも`apt.txt`を自動的に作成してくれれば...
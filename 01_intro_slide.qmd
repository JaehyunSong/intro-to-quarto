...
::: {.column width=70%}
- **氏名：**<ruby>宋<rt>そん</rt></ruby><ruby>財泫<rt>じぇひょん</rt></ruby>
- **所属：**関西大学総合情報学部　准教授
- 博士（政治学; 神戸大学）
...
- Webブラウザ[（Firefox、Chrome、Edge、Safari等）]{.small}はプレーンテキストであるHTMLをリッチテキストへ変換するエンジン[（Gecko、Blink、WebKit等）]{.small}を搭載している
...
```{r}
#| echo: false
qrcode::qr_code("https://www.jaysong.net/intro-to-quarto", ecl = "L") |> 
  plot(col = c(NA, "black"))
```
